...
- **Multi-context / multi-page not supported.** `browser.newContext()` and `ctx.newPage()` are intercepted by the adapter and throw a friendly error pointing back here. Underneath, ArkWeb's CDP doesn't implement `Target.createBrowserContext` / `Target.createTarget` in `connectOverCDP` mode — the raw `browser.newContext()` call actually returns an empty context (0 pages), but `ctx.newPage()` on it fails with `Cannot read properties of undefined (reading '_page')`. Either way, plan for one context + one page — isolate with `localStorage.clear()` + `page.reload()`. For device emulation use the `emulateDevice` fixture instead of `browser.newContext({ ...device })`.
