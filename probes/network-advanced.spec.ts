...
test('setExtraHTTPHeaders: 自定义头到达服务器', async ({ page }) => {
  ...
  try {
    ...
    const body = await page.goto(`http://${serverHost}:${port}/`).then(r => r!.text())
...
test('offline: context.setOffline(true)', async ({ page, context }) => {
  ...
  try {
    ...
    const result = await page.goto(`http://${serverHost}:${port}/`, { timeout: 3000 })
      .then(() => 'reachable')
      .catch((e: any) => `unreachable: ${e.message.split('\n')[0]}`)
    console.log(`[PROBE offline] RESULT when-offline=${result}`)
    await context.setOffline(false)
    const result2 = await page.goto(`http://${serverHost}:${port}/`, { timeout: 3000 })
      .then(() => 'reachable')
      .catch((e: any) => `unreachable: ${e.message.split('\n')[0]}`)
