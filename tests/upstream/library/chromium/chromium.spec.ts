...
import { contextTest as test, expect } from '../../fixtures/upstream-fixture.js';
import { playwrightTest } from '../../fixtures/upstream-fixture.js';
...
test('PLAYWRIGHT_DISABLE_SERVICE_WORKER_NETWORK', async ({ mode, context, page, server }) => {
  test.skip(mode !== 'default', 'no env in non-default mode');
  process.env.PLAYWRIGHT_DISABLE_SERVICE_WORKER_NETWORK = '1';

  const urls = [];
  context.on('request', r => {
    expect(r.serviceWorker()).toBe(null);
    urls.push(r.url());
  });
  ...
  expect(urls).toEqual([
    ...
    server.PREFIX + '/serviceworkers/fetch/style.css',
  ]);

  await page.evaluate(() => window['activationPromise']);
  await page.evaluate(() => fetch('./inner.txt'));
  expect(urls).toEqual([
    server.PREFIX + '/serviceworkers/fetch/sw.html',
    server.PREFIX + '/serviceworkers/fetch/style.css',
    server.PREFIX + '/serviceworkers/fetch/inner.txt',
  ]);
